...
resource "aws_cloudwatch_event_target" "ActivateModelTrainingStrategy" {
  target_id = "${local.prefix}-ActivateModelTrainingStrategy-${local.suffix}"
  rule      = aws_cloudwatch_event_rule.ActivateModelTraining.name
  arn       = aws_lambda_function.ml_lambda_functions["ActivateDatasetDevelopment"].arn
  input          = jsonencode(
  {
...
resource "aws_s3_bucket_notification" "sagemaker-bucket-ActivateModelTraining" {
  ...
  lambda_function {
    ...
    id                  = "${local.prefix}-sagemaker-bucket-ActivateModelTrainingMace-${local.suffix}"
    events              = ["s3:ObjectCreated:*"]
    filter_prefix       = "mace_data/mace"
    filter_suffix       = ".csv"
  }
  lambda_function {
    lambda_function_arn = aws_lambda_function.ml_lambda_functions["ActivateModelTraining"].arn
    id                  = "${local.prefix}-sagemaker-bucket-ActivateModelTrainingStrategy-${local.suffix}"
    events              = ["s3:ObjectCreated:*"]
    filter_prefix       = "strategy_data/strategy"
    filter_suffix       = ".csv"
  }
}
...
resource "aws_cloudwatch_event_target" "ActivatePredictionUpdateRoutineMace" {
  target_id = "${local.prefix}-ActivatePredictionUpdateRoutineMace-${local.suffix}"
...
resource "aws_cloudwatch_event_target" "ActivatePredictionUpdateRoutineStrategy" {
  target_id = "${local.prefix}-ActivatePredictionUpdateRoutineStrategy-${local.suffix}"
  rule      = aws_cloudwatch_event_rule.ActivatePredictionUpdateRoutine.name
  arn       = aws_lambda_function.ml_lambda_functions["ActivateUpdatePredictions"].arn
  input          = jsonencode(
  {
