...
resource "aws_db_instance" "rds-postgre" {
  ...
  backup_retention_period = local.suffix == "prod" ? 30 : 0
...
#RDS ETL Testing:
resource "aws_db_instance" "rds-postgre-etl" {
  identifier              = "${local.prefix}-rds-etl-test-db-${local.suffix}"
  name                    = "postgresetltest"
  allocated_storage       = 50
  max_allocated_storage   = 200
  storage_type            = "gp2"
  storage_encrypted       = var.storage_encrypted
  ...
  instance_class          = var.db_instance_class
  db_subnet_group_name    = aws_db_subnet_group.sg-rds-postgre.name
  username                = var.db-username-etl
  password                = var.db-password-etl
  backup_retention_period = local.suffix == "prod" ? 30 : 0
  #multi_az                = local.suffix == "prod" ? true : false //commenting since no HA is required
  multi_az               = false
  skip_final_snapshot    = true
  ...
  tags = merge(
  local.common_tags,
  tomap({ "Name" = "${local.prefix}-rds-etl-test-db-${local.suffix}" })
  )
}
