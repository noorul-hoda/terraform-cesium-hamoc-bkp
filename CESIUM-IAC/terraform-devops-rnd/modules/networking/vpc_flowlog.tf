locals {
  ...
  flow_log_iam_role_arn    = local.create_flow_log_cloudwatch_iam_role ? aws_iam_role.vpc_flow_log_cloudwatch[0].arn : var.flow_log_cloudwatch_iam_role_arn
}
...
resource "aws_flow_log" "flow_log" {
  ...
  tags = merge(
    local.common_tags,
    tomap({ "Name" = "${local.prefix}-vpc-flowlog-${local.suffix}" }
    )
  )
}
...
resource "aws_iam_role" "vpc_flow_log_cloudwatch" {
  ...
  name                 = "${local.prefix}-vpc-flow-log-role-${local.suffix}"
