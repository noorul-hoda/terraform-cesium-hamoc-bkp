...
resource "aws_s3_bucket_policy" "policy-cf-standard-logs" {
  bucket = aws_s3_bucket.cf-standard-logs.id
  depends_on = [aws_s3_bucket.cf-standard-logs]
  policy = jsonencode({
    Version = "2012-10-17"
    Id      = "CFStandardLogsPolicy"
    Statement = [
      {
        ...
        Action    = "s3:*"
        Resource = [
          aws_s3_bucket.cf-standard-logs.arn,
          "${aws_s3_bucket.cf-standard-logs.arn}/*",
        ]
        Condition = {
          Bool = {
