...
###########################
##uploadtos3_lambda_role
###########################

resource "aws_iam_role" "uploadtos3_lambda_role" {
  name               = "${local.prefix}-uploadtos3-lambda_role-${local.suffix}"
  assume_role_policy = file("${path.module}/lambda-assume-policy.json")
}

resource "aws_iam_role_policy_attachment" "AWSLambdaBasicExecutionRole_uploadtos3" {
  role       = aws_iam_role.uploadtos3_lambda_role.name
  policy_arn = aws_iam_policy.basic_lambda_execution_policy.arn
}

resource "aws_iam_policy" "uploadtos3_lambda_policy" {
  name   = "${local.prefix}-uploadtos3_lambda_policy-${local.suffix}"
  policy = <<EOF
{
  "Version" : "2012-10-17",
  "Statement": [
...
resource "aws_iam_role_policy_attachment" "uploadtos3_lambda_policy" {
  role       = aws_iam_role.uploadtos3_lambda_role.name
  policy_arn = aws_iam_policy.uploadtos3_lambda_policy.arn
}
