...
##########################
###externalapi Resources
##########################

resource "aws_api_gateway_resource" "externalapi" {
  rest_api_id = aws_api_gateway_rest_api.api.id
  parent_id   = aws_api_gateway_rest_api.api.root_resource_id
  path_part   = "externalapi"
}

resource "aws_api_gateway_resource" "externalapi_ProxyResource" {
  rest_api_id = aws_api_gateway_rest_api.api.id
  parent_id   = aws_api_gateway_resource.externalapi.id
  path_part   = "{proxy+}"
}

#externalapi PROXY-ANY

resource "aws_api_gateway_method" "externalapi_ProxyResource-ANY" {
  rest_api_id = aws_api_gateway_rest_api.api.id
  resource_id = aws_api_gateway_resource.externalapi_ProxyResource.id
  http_method = "ANY"
...
resource "aws_api_gateway_integration" "externalapi_ProxyResource-ANY" {
  rest_api_id             = aws_api_gateway_rest_api.api.id
  resource_id             = aws_api_gateway_resource.externalapi_ProxyResource.id
  http_method             = aws_api_gateway_method.externalapi_ProxyResource-ANY.http_method
  integration_http_method = "POST"
  type                    = "AWS_PROXY"
  passthrough_behavior    = "WHEN_NO_MATCH"
  content_handling        = "CONVERT_TO_TEXT"
  uri                     = var.externalapi_invoke_arn

  cache_key_parameters = ["method.request.path.proxy"]

}

resource "aws_api_gateway_integration_response" "externalapi_ProxyResource-ANY" {
  depends_on  = [aws_api_gateway_integration.externalapi_ProxyResource-ANY]
  rest_api_id = aws_api_gateway_rest_api.api.id
  resource_id = aws_api_gateway_resource.externalapi_ProxyResource.id
  http_method = aws_api_gateway_method.externalapi_ProxyResource-ANY.http_method
  status_code = "200"
...
#externalapi PROXY-OPTIONS

resource "aws_api_gateway_method" "externalapi_ProxyResource-OPTIONS" {
  rest_api_id = aws_api_gateway_rest_api.api.id
  resource_id = aws_api_gateway_resource.externalapi_ProxyResource.id
  http_method = "OPTIONS"
...
resource "aws_api_gateway_method_response" "externalapi_ProxyResource-OPTIONS" {
  rest_api_id = aws_api_gateway_rest_api.api.id
  resource_id = aws_api_gateway_resource.externalapi_ProxyResource.id
  http_method = aws_api_gateway_method.externalapi_ProxyResource-OPTIONS.http_method
  status_code = "200"
...
resource "aws_api_gateway_integration" "externalapi_ProxyResource-OPTIONS" {
  rest_api_id          = aws_api_gateway_rest_api.api.id
  resource_id          = aws_api_gateway_resource.externalapi_ProxyResource.id
  http_method          = aws_api_gateway_method.externalapi_ProxyResource-OPTIONS.http_method
  type                 = "MOCK"
...
resource "aws_api_gateway_integration_response" "externalapi_ProxyResource-OPTIONS" {
  depends_on  = [aws_api_gateway_integration.externalapi_ProxyResource-OPTIONS]
  rest_api_id = aws_api_gateway_rest_api.api.id
  resource_id = aws_api_gateway_resource.externalapi_ProxyResource.id
  http_method = aws_api_gateway_method.externalapi_ProxyResource-OPTIONS.http_method
  status_code = aws_api_gateway_method_response.externalapi_ProxyResource-OPTIONS.status_code
  response_templates = {
    "application/json" = <<EOF
  EOF
  }
...
##########################
###uploadtos3 Resources
##########################

resource "aws_api_gateway_resource" "uploadtos3" {
  rest_api_id = aws_api_gateway_rest_api.api.id
  parent_id   = aws_api_gateway_rest_api.api.root_resource_id
  path_part   = "uploadtos3"
}

resource "aws_api_gateway_resource" "uploadtos3_ProxyResource" {
  rest_api_id = aws_api_gateway_rest_api.api.id
  parent_id   = aws_api_gateway_resource.uploadtos3.id
  path_part   = "{proxy+}"
}

#uploadtos3 PROXY-ANY

resource "aws_api_gateway_method" "uploadtos3_ProxyResource-ANY" {
  rest_api_id = aws_api_gateway_rest_api.api.id
  resource_id = aws_api_gateway_resource.uploadtos3_ProxyResource.id
  http_method = "ANY"
...
resource "aws_api_gateway_integration" "uploadtos3_ProxyResource-ANY" {
  rest_api_id             = aws_api_gateway_rest_api.api.id
  resource_id             = aws_api_gateway_resource.uploadtos3_ProxyResource.id
  http_method             = aws_api_gateway_method.uploadtos3_ProxyResource-ANY.http_method
  integration_http_method = "POST"
  type                    = "AWS_PROXY"
  passthrough_behavior    = "WHEN_NO_MATCH"
  content_handling        = "CONVERT_TO_TEXT"
  uri                     = var.uploadtos3_invoke_arn

  cache_key_parameters = ["method.request.path.proxy"]

}

resource "aws_api_gateway_integration_response" "uploadtos3_ProxyResource-ANY" {
  depends_on  = [aws_api_gateway_integration.uploadtos3_ProxyResource-ANY]
  rest_api_id = aws_api_gateway_rest_api.api.id
  resource_id = aws_api_gateway_resource.uploadtos3_ProxyResource.id
  http_method = aws_api_gateway_method.uploadtos3_ProxyResource-ANY.http_method
  status_code = "200"
...
#uploadtos3 PROXY-OPTIONS

resource "aws_api_gateway_method" "uploadtos3_ProxyResource-OPTIONS" {
  rest_api_id = aws_api_gateway_rest_api.api.id
  resource_id = aws_api_gateway_resource.uploadtos3_ProxyResource.id
  http_method = "OPTIONS"
...
resource "aws_api_gateway_method_response" "uploadtos3_ProxyResource-OPTIONS" {
  rest_api_id = aws_api_gateway_rest_api.api.id
  resource_id = aws_api_gateway_resource.uploadtos3_ProxyResource.id
  http_method = aws_api_gateway_method.uploadtos3_ProxyResource-OPTIONS.http_method
  status_code = "200"
...
resource "aws_api_gateway_integration" "uploadtos3_ProxyResource-OPTIONS" {
  rest_api_id          = aws_api_gateway_rest_api.api.id
  resource_id          = aws_api_gateway_resource.uploadtos3_ProxyResource.id
  http_method          = aws_api_gateway_method.uploadtos3_ProxyResource-OPTIONS.http_method
  type                 = "MOCK"
...
resource "aws_api_gateway_integration_response" "uploadtos3_ProxyResource-OPTIONS" {
  depends_on  = [aws_api_gateway_integration.uploadtos3_ProxyResource-OPTIONS]
  rest_api_id = aws_api_gateway_rest_api.api.id
  resource_id = aws_api_gateway_resource.uploadtos3_ProxyResource.id
  http_method = aws_api_gateway_method.uploadtos3_ProxyResource-OPTIONS.http_method
  status_code = aws_api_gateway_method_response.uploadtos3_ProxyResource-OPTIONS.status_code
  response_templates = {
    "application/json" = <<EOF
  EOF
  }
